  ...
  useEffect(() => {
    const fetchVideo = async () => {
      ...
      try {
        ...
        let method: "youtube" | "file" | "url" = "youtube";
        if (video.youtubeUrl) {
          method = "youtube";
        } else if (video.uploadUrl) {
          // Check if it's a file upload (contains /uploads/videos/) or external URL
          if (video.uploadUrl.includes("/uploads/videos/") || video.uploadUrl.includes("supabase")) {
            method = "file";
            setVideoPreview(video.uploadUrl);
          } else {
            method = "url";
          }
        }
    ...
    return (
      <div className="min-h-screen bg-[#F3F4F6] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#4A5768]">Loading video...</p>
        </div>
      </div>
    );
  }
